...
#     new_df = df.drop(['Volume', 'Quote asset volume', 'Number of trades','Taker buy base asset volume', 'Taker buy quote asset volume'], axis=1, inplace=True)
#     return new_df
...
def render_macd():
    ...
    df = gd.api_data()
    # df = gd.static_data()
    df.drop(['Volume', 'Quote asset volume', 'Number of trades','Taker buy base asset volume', 'Taker buy quote asset volume'], axis=1, inplace=True)
    m = macd(df, 25, 12)
...
def oscillator_signal():
    ...
    coordinates = last4_df.values[-1].tolist()
    diff_macd_signal.all()
    return diff_macd_signal.all()

#%%
print(render_macd().tail())
